...
class Tpipes:
	...
	def updatezabbix(self):
		...
		for key in self.tpipedict:
			self.item = "/usr/bin/zabbix_sender -z 10.33.26.165 -s mob-sa-maillogs-L-1-r1 -k " + key + " -o " + str(self.tpipedict[key])
			self.bashing(self.item)

	def logcounter(self,tpipe):
		self.tpipe = tpipe
		self.cmd = "grep -c " + "\'" + self.tpipe + " 0 recieved data for\' " + logfile
